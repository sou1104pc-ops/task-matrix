...
DEFAULT_PROJECTS = [
    {"id": "p1", "name": "Brain コンテンツ販売", "desc": "レンタルスタジオ開業ガイド教材の制作・販売", "color": "#7b78ff"},
    {"id": "p2", "name": "姫路民泊投資", "desc": "姫路城周辺の短期賃貸物件の取得・運営", "color": "#ffc933"},
    {"id": "p3", "name": "M&A・事業売却", "desc": "3年以内に約1億円での事業売却を目指す", "color": "#2ecc71"},
    {"id": "p4", "name": "スタジオ運営改善", "desc": "各拠点のオペレーション効率化・品質向上", "color": "#48bfe3"},
]
...
DEFAULT_TASKS = [
    {"id": 1, "name": "emu-LABO 契約解除通知 最終確認", "desc": "法的整合性・支払い根拠の再確認", "quad": "A", "future": 4, "status": "doing", "due": "2026-03-08", "assignees": ["a"], "tag": "法務", "project": ""},
    {"id": 2, "name": "Brainコンテンツ スライド作成（第3章）", "desc": "トランスクリプトから図解スライドへ変換", "quad": "A", "future": 5, "status": "todo", "due": "2026-03-10", "assignees": ["a"], "tag": "コンテンツ", "project": "p1"},
    {"id": 3, "name": "神戸スタジオ 顧客対応マニュアル整備", "desc": "予約〜退室フローの標準化", "quad": "B", "future": 4, "status": "doing", "due": "2026-03-15", "assignees": ["a", "b"], "tag": "運営", "project": "p4"},
    {"id": 4, "name": "姫路民泊 物件調査レポート", "desc": "投資試算・競合分析を含む", "quad": "B", "future": 5, "status": "todo", "due": "2026-03-20", "assignees": ["c"], "tag": "投資", "project": "p2"},
    {"id": 5, "name": "M&A 売却シナリオ見直し（3年計画）", "desc": "EBITDA倍率・拠点数目標の更新", "quad": "B", "future": 5, "status": "todo", "due": "2026-04-01", "assignees": ["a"], "tag": "経営", "project": "p3"},
    {"id": 6, "name": "新大阪スタジオ 月次レポート提出", "desc": "3月分稼働率・売上集計", "quad": "C", "future": 2, "status": "todo", "due": "2026-03-31", "assignees": ["b", "d"], "tag": "経営", "project": "p4"},
    {"id": 7, "name": "加古川スタジオ 設備点検", "desc": "照明・音響・清掃用具チェック", "quad": "C", "future": 1, "status": "todo", "due": "2026-03-18", "assignees": ["d", "e"], "tag": "設備", "project": "p4"},
    {"id": 8, "name": "Brain セールスレター 改稿", "desc": "ターゲット訴求を強化", "quad": "B", "future": 5, "status": "todo", "due": "2026-03-22", "assignees": ["a"], "tag": "コンテンツ", "project": "p1"},
    {"id": 9, "name": "競合スタジオ NONO 調査メモ整理", "desc": "加古川・高砂・神戸3拠点の料金体系比較", "quad": "B", "future": 4, "status": "done", "due": "2026-02-20", "assignees": ["c", "a"], "tag": "調査", "project": "p4"},
]
...
def load_data():
    if os.path.exists(DATA_FILE):
        with open(DATA_FILE, "r", encoding="utf-8") as f:
            return json.load(f)
    return {"tasks": DEFAULT_TASKS, "projects": DEFAULT_PROJECTS, "members": DEFAULT_MEMBERS}
...
def task_card_html(t):
    ...
    ahtml = assignee_html(t.get("assignees", []))
    proj = get_project(t.get("project", ""))
    proj_html = ""
    if proj:
        proj_html = f'<span style="color:{proj["color"]};font-size:10px;">◆ {proj["name"]}</span> '
    tag = t.get("tag", "")
...
def apply_filters(tasks):
    ...
    if q:
        ql = q.lower()
        result = [t for t in result if ql in t["name"].lower() or ql in t.get("desc", "").lower()]
    ...
    if asns:
        result = [t for t in result if any(a in t.get("assignees", []) for a in asns)]
...
def render_sidebar():
    ...
    f["search"] = st.sidebar.text_input("テキスト検索", value=f.get("search", ""), key="sb_search")

    status_opts = ["すべて", "未着手", "進行中", "完了"]
    f["status"] = st.sidebar.selectbox("ステータス", status_opts,
                                        index=status_opts.index(f.get("status", "すべて")), key="sb_status")

    quad_opts = ["A", "B", "C", "D"]
    f["quads"] = st.sidebar.multiselect("象限", quad_opts, default=f.get("quads", []), key="sb_quads")
    ...
    f["future_min"] = st.sidebar.slider("未来重要度（最低）", 1, 5, value=f.get("future_min", 1), key="sb_future")
    ...
    for m in st.session_state.members:
        ...
        if c2.button("削除", key=f"del_m_{m['id']}"):
            ...
            for t in st.session_state.tasks:
                t["assignees"] = [a for a in t.get("assignees", []) if a != m["id"]]
            save_data()
            st.rerun()

    with st.form("add_member_form", clear_on_submit=True):
        nm = st.text_input("名前", key="new_m_name")
        nc = st.color_picker("カラー", value="#7b78ff", key="new_m_color")
        if st.form_submit_button("追加"):
            if nm.strip():
                new_id = f"m{len(st.session_state.members)+1}_{nm[:3]}"
                st.session_state.members.append({"id": new_id, "name": nm.strip(), "color": nc})
                save_data()
                st.rerun()
...
def render_project_manager():
    ...
    with st.form("add_project_form", clear_on_submit=True):
        pn = st.text_input("プロジェクト名", key="new_p_name")
        pd_input = st.text_input("説明", key="new_p_desc")
        pc = st.color_picker("カラー", value="#7b78ff", key="new_p_color")
        if st.form_submit_button("追加"):
            if pn.strip():
                new_pid = f"p{len(st.session_state.projects)+1}"
                st.session_state.projects.append({"id": new_pid, "name": pn.strip(), "desc": pd_input, "color": pc})
                save_data()
                st.rerun()
...
def render_task_form(task=None, form_key="add_task"):
    ...
    with st.form(form_key, clear_on_submit=not is_edit):
        ...
        with col1:
            ...
            status = st.selectbox("ステータス", status_labels,
                                  index=status_opts.index(cur_status))
            status = status_opts[status_labels.index(status)]
        ...
        member_opts = {m["id"]: m["name"] for m in st.session_state.members}
        cur_assignees = [member_opts[a] for a in task.get("assignees", []) if a in member_opts] if is_edit else []
        ...
        submitted = st.form_submit_button("保存" if is_edit else "追加")
        if submitted:
            if not name.strip():
                st.error("タスク名は必須です")
                return False
            ...
            if is_edit:
                task.update({
                    "name": name.strip(), "desc": desc, "quad": quad,
...
def render_stats(tasks_all):
    todo_n = sum(1 for t in tasks_all if t["status"] == "todo")
    doing_n = sum(1 for t in tasks_all if t["status"] == "doing")
    done_n = sum(1 for t in tasks_all if t["status"] == "done")
    a_remain = sum(1 for t in tasks_all if t["quad"] == "A" and t["status"] != "done")

    c1, c2, c3, c4 = st.columns(4)
    c1.metric("未着手", todo_n)
    c2.metric("進行中", doing_n)
    c3.metric("完了", done_n)
    c4.metric("A象限 残タスク", a_remain)

# ── Kanban view ───────────────────────────────────────────────────────────────
def render_kanban(tasks):
    cols = st.columns(3)
    statuses = [("todo", "未着手", "#888"), ("doing", "進行中", "#7b78ff"), ("done", "完了", "#2ecc71")]
    for i, (skey, slabel, scolor) in enumerate(statuses):
        with cols[i]:
            st.markdown(
                f'<div class="col-header" style="background:#1a1a2e;color:{scolor};">{slabel}</div>',
                unsafe_allow_html=True,
            )
            group = [t for t in tasks if t["status"] == skey]
            for t in group:
    ...
    for row_quads in quads:
        ...
        for col_idx, q in enumerate(row_quads):
            with cols[col_idx]:
                ...
                label = QUAD_LABELS[q]
                st.markdown(
                    f'<div class="quad-badge badge-{q}">{label}</div>',
                    unsafe_allow_html=True,
                )
                group = [t for t in tasks if t["quad"] == q]
                for t in group:
                    st.markdown(task_card_html(t), unsafe_allow_html=True)
                    ec1, ec2 = st.columns([1, 1])
...
<div class="proj-card" style="border-top-color:{p['color']}">
  ...
  <div style="color:#6060a0;font-size:12px;margin-top:4px;">{p['desc']}</div>
</div>
""", unsafe_allow_html=True)
...
def render_table(tasks):
    ...
    for t in tasks:
        rows.append({
            ...
            "締切日": t.get("due", ""),
            "担当者": ", ".join(member_map.get(a, a) for a in t.get("assignees", [])),
            "タグ": t.get("tag", ""),
...
def main():
    ...
    with tab1:
        render_kanban(filtered)

    with tab2:
        render_matrix(filtered)

    with tab3:
        render_projects(filtered)
